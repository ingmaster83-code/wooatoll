...
def gen_gate_page(dep_name, dep_slug, routes_from):
    title = f"{dep_name} 통행료 — 목적지별 요금 {YEAR} | {SITE_NAME}"
    desc = f"{dep_name}에서 출발하는 고속도로 통행료를 목적지별로 확인하세요. 총 {len(routes_from)}개 목적지."
    canonical = f"{BASE_URL}/{quote(dep_slug)}/index.html"

    routes_from = sorted(routes_from, key=lambda r: r["arr"])
    links = "".join(
        f'<a href="{quote(r["arrSlug"])}.html" style="display:flex;justify-content:space-between;'
        f'padding:10px 12px;border:1px solid #E5E7EB;border-radius:8px;margin-bottom:6px;text-decoration:none;color:inherit;">'
        f'<span>{r["arr"]}</span><span style="color:#6B7280;font-size:.85rem;">{fmt_won(r["charges"][0])}~</span></a>'
  ...
  <p style="color:#6B7280;font-size:.9rem;margin-bottom:20px;">{dep_name}에서 출발하는 목적지 {len(routes_from)}곳의 통행료(1종 기준)</p>
...
def gen_index(gates, total_routes):
    title = f"전국 고속도로 통행료 조회 {YEAR} | {SITE_NAME}"
    desc = f"전국 요금소 {len(gates)}곳, {total_routes:,}개 구간의 고속도로 통행료를 차종별로 무료 조회하세요."
    canonical = f"{BASE_URL}/"

    gate_options = "".join(f'<option value="{quote(g["slug"])}">{g["name"]}</option>' for g in sorted(gates, key=lambda x: x["name"]))
...
<div class="container" style="padding:20px 16px 40px;max-width:680px;margin:0 auto;">
  ...
  <div style="background:#F9FAFB;border-radius:12px;padding:18px;margin:16px 0 28px;">
    ...
    <select id="depSel" style="width:100%;padding:10px;margin:6px 0 12px;border-radius:8px;border:1px solid #E5E7EB;">
      <option value="">선택하세요</option>{gate_options}
    </select>
    ...
    <p id="err" style="display:none;color:#DC2626;font-size:.82rem;margin-top:8px;">해당 구간 데이터가 없습니다. 요금소를 다시 선택해주세요.</p>
  </div>
...
function goRoute(){{
  ...
  if(!d || !a){{return;}}
  if(d === a){{document.getElementById('err').style.display='block';return;}}
  location.href = d + '/' + a + '.html';
}}
</script>
"""
    head = page_head(title, desc, canonical, "")
...
def main():
    ...
    for (dep_name, dep_slug), rlist in by_dep.items():
        ...
        generated += 1
        for r in rlist:
            (gate_dir / f"{r['arrSlug']}.html").write_text(gen_route_page(r), encoding="utf-8")
            url_paths.append(f"{dep_slug}/{r['arrSlug']}.html")
            generated += 1
        if generated % 20000 < len(rlist) + 1:
            print(f"  진행: {generated:,}개 생성...")

    (DOCS_DIR / "index.html").write_text(gen_index(gates, len(routes)), encoding="utf-8")
    generated += 1
...
def gen_sitemap(paths):
    ...
    for n, chunk in enumerate(chunks, 1):
        ...
        (DOCS_DIR / f"sitemap-{n}.xml").write_text(xml, encoding="utf-8")
        idx_entries.append(f"  <sitemap><loc>{BASE_URL}/sitemap-{n}.xml</loc></sitemap>")
    idx_xml = ('<?xml version="1.0" encoding="UTF-8"?>\n<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n'
               + "\n".join(idx_entries) + "\n</sitemapindex>\n")
    (DOCS_DIR / "sitemap_index.xml").write_text(idx_xml, encoding="utf-8")
    print(f"  sitemap: {len(chunks)}개 파일 분할, 총 {len(paths):,}개 URL")
